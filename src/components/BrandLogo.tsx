...
export default function BrandLogo({ light = false, href = '#' }: BrandLogoProps) {
  ...
  return (
    <a href={href} className="brand-logo" aria-label="Noema Carmen Capli Inicio" onClick={(e) => { e.preventDefault(); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>
      ...
      <div className="brand-text-block">
        <span className="brand-title" style={light ? { color: '#FAF8F5' } : undefined}>CARMEN CAPLI</span>
        <div className="brand-subtitle-line">
          <span className="brand-dot"></span>
        </div>
        <span className="brand-subtitle" style={light ? { color: 'rgba(250, 248, 245, 0.7)' } : undefined}>N O E M A</span>
      </div>
    </a>
  )
}
